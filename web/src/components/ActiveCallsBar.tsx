...
import { callPhase, type Call } from "@/lib/types";
...
export function ActiveCallsBar() {
  ...
  const apply = useCallback((call: Call) => {
    setCalls((previous) => {
      ...
      // Newest first, and an ended call leaves the strip rather than lingering.
  ...
  useLiveEvents(
    {
      "call.started": apply,
      "call.updated": (payload) => apply(payload.call),
    },
    load,
  );
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        {calls.map((call) => (
          <Link
            key={call.id}
            href={`/calls/${call.id}`}
            className="group flex min-w-0 items-center gap-2.5 rounded-md border border-green-400/20 bg-canvas/40 px-2.5 py-1 text-[12px] transition-colors hover:border-green-400/40 hover:bg-canvas/70"
          >
            ...
            <PhaseTrack phase={callPhase(call)} compact />
            {call.caller_phone ? (
